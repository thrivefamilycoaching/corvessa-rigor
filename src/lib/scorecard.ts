...
// RULE 3 — Odds-to-label sync: <30% → Reach, 30-65% → Match, >65% → Safety.
...
function classifyDeterministic(
  ...
  if (personalizedOdds > 65) category = "safety";
...
function normalizeDisplayOdds(school: RecommendedSchool): RecommendedSchool {
  ...
  if (school.type === "match") {
    if (odds < 30 || odds > 65) {
      displayOdds = 30 + (nameHash(school.name) % 36); // 30–65
    }
    ...
    if (odds <= 65) {
      displayOdds = 66 + (nameHash(school.name) % 30); // 66–95
    }
  }
...
function pick343(pool: RecommendedSchool[]): RecommendedSchool[] {
  ...
  const MIDPOINTS: Record<string, number> = { reach: 15, match: 48, safety: 80 };

...
// RULE 3 — Odds-to-label sync: <40% → Reach, 40-74% → Match, ≥75% → Safety.
...
function classifyDeterministic(
  ...
  else if (personalizedOdds >= 40) category = "match";
  else category = "reach";
...
export async function enrichSchoolsWithScorecardData(
  ...
  const enriched = toEnrich.map((school, i) => {
    ...
    // Sub-25% admit rate → cap at 39% (guaranteed Reach via <40% guard)
    if (admissionRate < 0.25) {
      finalOdds = Math.min(finalOdds, 39);
    }

    // DETERMINISTIC CLASSIFICATION — data overrides AI, no exceptions
    const type = classifyDeterministic(school.name, finalOdds, admissionRate, studentGPA);

    // FINAL SAFETY NET: if odds < 40%, label CANNOT be Safety or Match
    // This prevents any "9% chance — Safety" anomalies
    if (finalOdds < 40 && type !== "reach") {
      console.log(`[Scorecard] ${school.name}: SAFETY NET — odds=${finalOdds}% too low for "${type}", forcing reach`);
    }
    const guardedType = finalOdds < 40 ? "reach" : type;
...
//   Reach  → capped below 40%
//   Match  → clamped to 40–74%
//   Safety → left as-is (naturally 75–95%)
...
function normalizeDisplayOdds(school: RecommendedSchool): RecommendedSchool {
  ...
  if (school.type === "match") {
    if (odds < 40 || odds >= 75) {
      displayOdds = 40 + (nameHash(school.name) % 35); // 40–74
    }
  } else if (school.type === "reach") {
    if (odds >= 40) {
      displayOdds = 5 + (nameHash(school.name) % 35); // 5–39
    }
